<?php
	...
	[字符串] $host [数据库服务器地址]
	[字符串] $code [数据传输编码] */
	function con($name,$psd,$db,$host='localhost',$code="utf-8"){
		//连接数据库服务器并选择数据库
		$_SESSION['mysqli']=mysqli_connect($host,$name,$psd,$db)or doe('链接数据库失败');
		//设置数据传输编码	
 ...
 */
 function getOne($table,$where,$field='*'){
	$sql="select $field from $table where"; //组装sql语句
...
function add($table,$data){
	...
	echo $sql="insert into $table ($feild) values ($values)"; //组装sql语句
